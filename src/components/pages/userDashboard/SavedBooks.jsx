...
export const SavedBooks = ({ handleStatus, setStatusTitle, setStatusMessage, setStatusColor }) => {
  ...
  const handleRemoveFromWishList = async (id) => {
    ...
    try {
      await axios.delete(`/book/save/remove/${id}`, {
        headers: {
          'Authorization': `Bearer ${JSON.parse(localStorage.getItem("userData")).accessToken}`
        }
      }).then(
                      ...
                      <div className="flex flex-col items-stretch w-4/5 max-md:w-full max-md:ml-0">
                        <span className="flex grow flex-col max-md:max-w-full max-md:mt-3">
                          ...
                          <span className="items-stretch flex justify-between gap-5 mt-6 self-start max-md:max-w-full max-md:flex-wrap">
                            <span onClick={() => (handleAddToCart(book?.id))} className="hover:bg-green-600 hover:text-white cursor-pointer text-green-500 text-base font-medium leading-5 uppercase justify-center items-stretch grow px-11 py-5 rounded-md border-[1.145px] border-solid border-green-600 max-md:px-5">
                              ADD TO CART
                            </span>
                            <div onClick={() => (handleRemoveFromWishList(book?.id))} className="hover:bg-red-600 hover:text-white cursor-pointer text-red-500 text-sm font-semibold leading-5 my-auto px-11 py-5 rounded-md border-[1.145px] border-solid border-red-600 max-md:px-5">
