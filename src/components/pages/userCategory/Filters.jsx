    ...
    return (
        <form className="justify-center items-stretch bg-white flex max-w-[18rem] flex-col px-8 py-7 rounded-md">
            ...
            <span className="items-center flex flex-col justify-between gap-3 mt-4">
                {renderRadio("1_STAR", "starRating", "1 Star", "1 Star", selectedRating, handleRadioChange)}
                {renderRadio("2_STARS", "starRating", "2 Stars", "2 Stars", selectedRating, handleRadioChange)}
                {renderRadio("3_STARS", "starRating", "3 Stars", "3 Stars", selectedRating, handleRadioChange)}
                {renderRadio("4_STARS", "starRating", "4 Stars", "4 Stars", selectedRating, handleRadioChange)}
                {renderRadio("5_STARS", "starRating", "5 Stars", "5 Stars", selectedRating, handleRadioChange)}
        </span>
            <span className="flex justify-between gap-1.5 mt-6 pr-4 items-start">
...
const renderCheckbox = (inputName, label, value) => (
    <div className="items-stretch flex w-full justify-between gap-2">
        ...
        />
        <div className="overflow-hidden text-xl text-zinc-900 text-ellipsis whitespace-nowrap self-stretch grow">
            {label}
        </div>
    </div>
);

const renderRadio = (inputId, inputName, label, value, selectedValue, handleRadioChange) => (
    <div className="items-stretch flex w-full justify-between gap-2">
        <input
            type="radio"
            id={inputId}
            ...
            className="aspect-[0.75] object-contain object-center w-[15px] overflow-hidden shrink-0 max-w-full bg-green-500 cursor-pointer"
        />
        <label htmlFor={inputId} className="overflow-hidden text-zinc-900 text-ellipsis whitespace-nowrap text-xl self-stretch grow">
            {label}
        </label>
    </div>
);
