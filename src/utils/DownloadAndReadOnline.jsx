...
export const DownloadAndReadOnline = ({ book, handleStatus, setStatusTitle, setStatusMessage, setStatusColor, subscription, isPurchased }) => {
    ...
    const handleSaveBook = async (e) => {
        ...
        try {
            await axios.get(`/book/save/${book.id}`, useConfig()).then(
                response => {
    ...
    return(
        <div className="flex gap-2.5 mt-6 self-start items-start max-md:max-w-full max-md:flex-wrap">
            ...
            { subscription === "VORACIOUS" || isPurchased ?
                ...
                :
                <span onClick={handleOptionClick} className="bg-blue-200 hover:bg-blue-500 hover:text-white cursor-pointer text-green-800 text-base font-medium leading-5 uppercase justify-center items-stretch bg-white grow px-8 py-5 rounded-md border-[1.145px] border-solid border-green-600 max-md:px-5">
            ...
            <div onClick={handleSaveBook} className="px-12 transition hover:border-solid hover:border-green-600 cursor-pointer h-[3.8rem] text-center py-[1.2rem] text-rose-500 text-base font-medium leading-5 uppercase justify-center items-stretch self-stretch grow rounded-md max-md:px-5 border-[1.174px] border-white">
                ADD TO SAVED
            </div>
        </div>
    )
